...
class APIListAudienciaTestCase(TestCase):
    ...
    def test_api_audiencia_list(self):
        ...
        date_exhibition_date = datetime.datetime.strptime(result_audiencia["exhibition_date"], "%Y-%m-%d").date()
        ...
        self.assertEqual(Decimal(result_audiencia["average_audience"]), audiencia.average_audience)
...
class APIListTempoDisponivelTestCase(TestCase):
    ...
    def test_api_tempo_disponivel_list(self):
        ...
        result_audiencia = result_json["results"][0]
        date_date = datetime.datetime.strptime(result_audiencia["date"], "%Y-%m-%d").date()
        self.assertEqual(result_json["count"], 1)
        self.assertEqual(result_audiencia["signal"], audiencia.signal)
        self.assertEqual(result_audiencia["program_code"], audiencia.program_code)
        ...
        mock_list = [
            {
                ...
                "weekday": 6,
                "available_time": {
                    "mean": 1, "median": 2, "sum": 3, "max": 4, "min": 5,
                },
                "predicted_audience": {
                    "mean": 6, "median": 7, "sum": 8, "max": 9, "min": 10,
                },
            }
        ]
...
class APIListAnaliticoTestCase(TestCase):
    ...
    def test_api_list_analitico_por_programa_retorna_sucesso(self):
        ...
        self.assertEqual(response.status_code, HTTPStatus.OK)
        result_json = json.loads(response.content.decode("utf-8"))
        mock_result = {'signal': 'SP1', 'program_code': 'PTV1', 'weekday': 4, 'available_time': {'mean': 933891.96, 'median': 933891.96, 'sum': 933891.96, 'max': 933891.96, 'min': 933891.96}, 'predicted_audience': {'mean': 10.0, 'median': 10.0, 'sum': 10, 'max': 10, 'min': 10}}
    ...
    def test_api_list_analitico_por_periodo_retorna_sucesso(self):
        ...
        response = self.client.get("/api/analitico/periodo/?exhibition_date_inicio=05%2F06%2F2020&exhibition_date_fim=05%2F06%2F2020")
        self.assertEqual(response.status_code, HTTPStatus.OK)
        result_json = json.loads(response.content.decode("utf-8"))
        mock_result = {'signal': 'SP1', 'program_code': 'PTV1', 'weekday': 4, 'available_time': {'mean': 933891.96, 'median': 933891.96, 'sum': 933891.96, 'max': 933891.96, 'min': 933891.96}, 'predicted_audience': {'mean': 10.0, 'median': 10.0, 'sum': 10, 'max': 10, 'min': 10}}
        self.assertEqual(result_json[0], mock_result)
